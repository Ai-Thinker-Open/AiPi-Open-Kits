...
static void tcp_client_socket_test(void)
{
    ...
    while (1) {
        ...
        printf("Total send data=0x%08lx%08lx\r\n",
               (uint32_t)((total_cnt >> 32) & 0xffffffff),
               (uint32_t)(total_cnt & 0xffffffff));
        closesocket(sock);
    }
}
...
{
    printf("connect error! close by core!\r\n");
...
static err_t tcp_client_recv(void *arg, struct tcp_pcb *t_pcb, struct pbuf *p, err_t err)
{
    if (p != NULL) {
        ...
        // disconnect server
        printf("server has been disconnect!\r\n");
        tcp_close(t_pcb);
...
void tcp_client_raw_init(void)
{
    ...
    client_pcb = tcp_new();
    if (client_pcb == NULL) {
        printf("creat client pcb failed!\r\n");
        return;
    }
