...
void axk_blufi_adv_start(void)
{
    ...
    else
    {
        ...
        memset(g_blufi_config.ble.blufi.blufiname, 0, BLE_ADV_DATA_MAX_LEN);
        memcpy(g_blufi_config.ble.blufi.blufiname, name, strlen(name));
    }
...
void blufi_set_name(const char* blufi_name)
{
    if (blufi_name==NULL) {
        return;
    }
    memset(g_blufi_config.ble.blufi.blufiname, 0, BLE_ADV_DATA_MAX_LEN);
    memcpy(g_blufi_config.ble.blufi.blufiname, blufi_name, strlen(blufi_name));
}

char* blufi_get_name(void)
{
    if (strlen(g_blufi_config.ble.blufi.blufiname)!=0)
        return g_blufi_config.ble.blufi.blufiname;
    else {
        return "AXK_BLUFI";
    }
}