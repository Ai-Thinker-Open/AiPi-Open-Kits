...
static lv_obj_t *img_bagua = NULL;
static void anim_timer_handle(lv_obj_t *parent)
{
    ...
    if (-90 == count) {
        ...
        lv_img_set_src(img_logo, &_logo_alpha_165x56);
        // lv_img_set_src(img_logo, &ai_logo0);
...
void boot_animate_start(lv_obj_t *parent)
{
    ...
    const lv_color_t arc_color[] = {
        LV_COLOR_MAKE(210, 230, 230),
        LV_COLOR_MAKE(240, 200, 200),
        LV_COLOR_MAKE(210, 200, 220),
    };
    for (size_t i = 0; i < sizeof(arc) / sizeof(arc[0]); i++) {
