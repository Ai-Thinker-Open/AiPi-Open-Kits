...
static ui_menu_app_t menu[] = {
    {"洗衣模式",    "Washer",      &ui_img_washer5_png,       LV_COLOR_MAKE(36, 163, 235), &washer_Layer},
    {"恒温器",      "A/C",      &ui_img_ac2_png,    LV_COLOR_MAKE(249, 139, 122), &ac_Layer},
    {"照明模式",    "Light",        &ui_img_led6_png,         LV_COLOR_MAKE(255, 229, 147), &light_rgb_Layer},
    {"遥控器",      "TV",         &ui_img_tv_png,         LV_COLOR_MAKE(248, 128, 248), &remote_Layer},
};

#define APP_NUM (sizeof(menu) / sizeof(ui_menu_app_t))
...
{
    if (num >= max) {
        LV_LOG_INFO("[ERROR] num should less than max");
        return num;
    }
...
static void menu_event_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (is_time_out(&time_100ms)) {
            ...
            //     factory_Enter = 7;
            //     LV_LOG_INFO("Invalid Enter factory");
            // }

            // if ((factory_Enter < 6) && (++factory_Enter == 6) && (app_index == 0)) {
            //     LV_LOG_INFO("Enter factory");
...
void ui_menu_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_bg_color(page, lv_color_hex(0x1A1A1A), LV_PART_MAIN | LV_STATE_DEFAULT);
