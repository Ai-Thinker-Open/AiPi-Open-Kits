...
static void wakeup_event_cb(lv_event_t *e)
{
    ...
    LV_LOG_USER("WAKEUP", "code:%d", code);
...
static void bagua_angle_cb(void *img, int32_t v)
{
...
void ui_flash_face_init(lv_obj_t *parent)
{
    ...
    lv_img_set_src(img_face, &bagua);
    lv_obj_center(img_face);
#if 0
    img_face = lv_img_create(page);
    ...
    lv_anim_t a;
    lv_anim_init(&a);
    lv_anim_set_var(&a, img_face);
    lv_anim_set_time(&a, 20000);
    lv_anim_set_exec_cb(&a, bagua_angle_cb);
    lv_anim_set_values(&a, 0, 3600);
    lv_anim_set_playback_time(&a, 20000);
