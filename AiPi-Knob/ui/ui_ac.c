...
#define TEMP_VALUE_MIN 16

static lv_obj_t *temp_arc;
static lv_obj_t *page;
static lv_obj_t *temp_wheel;
static lv_obj_t *temp_btn;
static lv_obj_t *label_A_C;
static time_out_count time_500ms;

static bool ac_layer_enter_cb(void *layer);
static bool ac_layer_exit_cb(void *layer);
static void ac_layer_timer_cb(lv_timer_t *tmr);

lv_layer_t ac_Layer = {
    .lv_obj_name    = "ac_Layer",
    .lv_obj_parent  = NULL,
    .lv_obj_layer   = NULL,
    .lv_show_layer  = NULL,
    .enter_cb       = ac_layer_enter_cb,
    .exit_cb        = ac_layer_exit_cb,
    .timer_cb       = ac_layer_timer_cb,
};

static void ac_event_cb(lv_event_t *e)
{
    static uint8_t current = 26;
...
#endif
            if (LV_KEY_RIGHT == key) {
                if (current < TEMP_VALUE_MAX) {
                    current++;
                }
            } else {
                if (current > TEMP_VALUE_MIN) {
                    current--;
                }
            }
            lv_roller_set_selected(temp_wheel, (current - TEMP_VALUE_MIN), LV_ANIM_ON);
        }
 ...
 */
static void lv_create_obj_roller(lv_obj_t *parent)
{
...
void ui_ac_init(lv_obj_t *parent)
{
    ...
    lv_obj_t *img_ac_bg = lv_img_create(page);
    lv_img_set_src(img_ac_bg, &AC_BG);
    lv_obj_align(img_ac_bg, LV_ALIGN_CENTER, 0, 0);

    // lv_obj_t *img_ac_temp = lv_img_create(page);
    // lv_img_set_src(img_ac_temp, &AC_temper);
    // lv_obj_align(img_ac_temp, LV_ALIGN_CENTER, 0, 20);
...
//     lv_obj_align(temp_arc, LV_ALIGN_TOP_MID, 0, 15);
    ...
    // lv_anim_t a2;
    // lv_anim_init(&a2);
    // lv_anim_set_var(&a2, img_ac_temp);
    // lv_anim_set_values(&a2, lv_obj_get_y_aligned(img_ac_temp) + 20, lv_obj_get_y_aligned(img_ac_temp));
    ...
    lv_obj_align(img_temp_unit, LV_ALIGN_CENTER, 80, -10);
    lv_create_obj_roller(parent);
    lv_roller_set_selected(temp_wheel, (22 - 19), LV_ANIM_ON);

    temp_btn = lv_btn_create(parent);
    lv_obj_set_width(temp_btn, 60);
    lv_obj_set_height(temp_btn, 60);
    lv_obj_add_flag(temp_btn, LV_OBJ_FLAG_SCROLL_ON_FOCUS);     /// Flags
    lv_obj_clear_flag(temp_btn, LV_OBJ_FLAG_SCROLLABLE);      /// Flags
    lv_obj_set_style_bg_color(temp_btn, lv_color_hex(0xB7C1C1), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(temp_btn, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(temp_btn, 50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(temp_btn, LV_ALIGN_CENTER, -40, 10);
    lv_obj_add_event_cb(temp_btn, ac_event_cb, LV_EVENT_ALL, NULL);
    label_A_C = lv_label_create(parent);
    lv_obj_set_style_text_color(label_A_C, lv_color_hex(COLOUR_GREY_4F), 0);
    lv_obj_set_style_text_font(label_A_C, &lv_font_montserrat_16, 0);
    lv_label_set_text(label_A_C, "OFF");
    lv_obj_align(label_A_C, LV_ALIGN_CENTER, -40, 10);


    ui_remove_all_objs_from_encoder_group();//roll will add event default.
    lv_obj_add_event_cb(page, ac_event_cb, LV_EVENT_FOCUSED, NULL);
    lv_obj_add_event_cb(page, ac_event_cb, LV_EVENT_KEY, NULL);
    lv_obj_add_event_cb(page, ac_event_cb, LV_EVENT_LONG_PRESSED, NULL);
    lv_obj_add_event_cb(page, ac_event_cb, LV_EVENT_CLICKED, NULL);
...
static bool ac_layer_enter_cb(void *layer)
{
    ...
    if (NULL == create_layer->lv_obj_layer) {
        ...
        ui_ac_init(create_layer->lv_obj_layer);
