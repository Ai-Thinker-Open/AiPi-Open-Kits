...
void ES8388_Codec_Mode(ES8388_Cfg_Type *cfg)
{
    ...
    if (cfg->mic_input_mode == ES8388_SINGLE_ENDED_MIC) {
        ...
        //mic2 select
        // ES8388_Write_Reg(0x0A, 0xfc); // Fixed stereo problems
        // ES8388_Write_Reg(0x0B, 0x82); // stereo
        #define USE_MIC_IN1_SINGLE	(0)
		#define USE_MIC_IN2_SINGLE	(0)
        #if USE_MIC_IN1_SINGLE
	        //mic1 select 
    	    ES8388_Write_Reg(0x0A, 0xf0); // Fixed stereo problems
	        ES8388_Write_Reg(0x0B, 0x02); // stereo
	    #elif USE_MIC_IN2_SINGLE
			//mic2 select 
			ES8388_Write_Reg(0x0A, 0xfc); // Fixed stereo problems
			ES8388_Write_Reg(0x0B, 0x82); // stereo
		#else //IN1+IN2
