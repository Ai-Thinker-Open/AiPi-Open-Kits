...
static void _connected(struct bt_conn* conn, u8_t err)
{
    ...
    if (state_task!=NULL)
        xTaskNotify((xTaskHandle)state_task, BLE_STATUS_CONNECT, eSetValueWithOverwrite);
    else LOG_E("state_task is no Create");
}
/**
...
static void _disconnected(struct bt_conn* conn, u8_t reason)
{
    ...
    if (state_task!=NULL)
        xTaskNotify((xTaskHandle)state_task, BLE_STATUS_DISCONNECT, eSetValueWithOverwrite);
    else LOG_E("state_task is no Create");
}
/**
 * @brief _le_param_req
...
static void bt_enable_cb(int err)
{
    ...
    if (!err) {
        ...
        //向界面发送状态
        if (state_task!=NULL)
            xTaskNotify((xTaskHandle)state_task, BLE_STATUS_ENABLE, eSetValueWithOverwrite);
        else LOG_E("state_task is no Create");
    }
}
/**
...
static void ble_hid_dev_send(hid_key_num_t key_num)
{
    ...
    switch (key_num)
    {
        ...
        break;
        case HID_KEY_NUMBLE_ARROW_RIGHT:
        {
            key_vaule[0] = KEY_NUMBLE_RIGHTARRTOW;
        ...
        break;
        case HID_KEY_NUMBLE_ARROW_LEFT:
        {
            key_vaule[0] = KEY_NUMBLE_LEFTTARTOW;
            hog_send_keyboard_value(ble_conn_handle, 0, key_vaule, sizeof key_vaule);
            vTaskDelay(50/portTICK_RATE_MS);
            hog_send_keyboard_value(ble_conn_handle, 0, NULL, 0);
            LOG_I("HID SEND:0X%02X", key_vaule[0]);
...
void ble_hid_task(void* arg)
{
    ...
    while (1) {
        xTaskNotifyWait(0xffffffff, 0x00, &kb_num, portMAX_DELAY);
