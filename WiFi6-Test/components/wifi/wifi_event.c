...
void wifi_event_handler(uint32_t code)
{
    ...
    switch (code) {
        ...
        case CODE_WIFI_ON_GOT_IP:
        {
            ...
            user_state_send_notify(STATE_WIFI_CONNECTED_OK, 1);
        }
        break;
        case CODE_WIFI_ON_DISCONNECT:
        {
            LOG_I("[APP] [EVT] %s, CODE_WIFI_ON_DISCONNECT", __func__);
            user_state_send_notify(STATE_WIFI_DISCONNECTED, 1);
        }
        break;
...
uint8_t wifi_connect(char* ssid, char* passwd, uint8_t channel)
{
    int ret = 255;
    // struct fhost_vif_ip_addr_cfg ip_cfg = { 0 };
    uint32_t ipv4_addr = 0;
    user_state_send_notify(STATE_WIFI_CONNECTING, 1);
    ...
    if (wifi_mgmr_sta_quickconnect(ssid, passwd, 2407+channel*5, 0)) {
        return 4;
    }
    // if (wifi_sta_connect(ssid, passwd, NULL, NULL, 0, 2407+channel*5, 0, 1)) {
    //     return 4;
    // }
    ...
    //             user_state_send_notify(STATE_WIFI_CONNECTING);
    //             break;
    //         case CODE_WIFI_ON_GOT_IP:
    //             user_state_send_notify(STATE_WIFI_CONNECTED_OK);
    //             return 0;
    //         default:
    //             //等待连接成功
    //             break;
