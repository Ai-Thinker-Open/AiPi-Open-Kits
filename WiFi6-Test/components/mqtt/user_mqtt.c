...
static void publish_callback_1(void** unused, struct mqtt_response_publish* published)
{
    ...
    strcpy(mq_msg.data, topic_msg);
    strcpy(mq_msg.topic, topic_name);
    user_state_send_notify(STATE_MQTT_MSG_ARRIVES, true);
...
void  mqtt_client_init(const char* host, int port)
{
    ...
    char* port_str = pvPortMalloc(5);
    memset(port_str, 0, 5);
    lwip_itoa(port_str, 5, port);

    test_sockfd = open_socket(host, port_str);
    if (test_sockfd < 0) {
...
{
    user_state_send_notify(STATE_MQTT_SERVER_CONENCT_OK, false);
    while (1)
    {
...
int mqtt_start_connect(char* host, uint16_t port, char* user_name, char* pass)
{
    ...
    char* mqtt_client = pvPortMalloc(64);
    memset(mqtt_client, 0, 64);
    wifi_mgmr_sta_mac_get(sta_mac);

    sprintf(mqtt_client, MQTT_CLIENT_ID, sta_mac[4], sta_mac[5]);

    ret = mqtt_connect(&client, mqtt_client, NULL, NULL, 0, user_name, pass, connect_flags, 120);
    if (ret != MQTT_OK)
    {
