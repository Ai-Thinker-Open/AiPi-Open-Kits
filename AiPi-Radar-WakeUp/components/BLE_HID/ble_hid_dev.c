...
#define BLE_DEV_NAME "AiPi-Keyboard"

#define BLE_USE_WINDOWS_BEACON 

static bool ble_start_status = false;

static xTaskHandle ble_send_task;
...
static void ble_stack_start(void)
{
    ...
    hci_driver_init();
    // if (guider_ui.keyboard_con_type) {
...
static void ble_hid_dev_send(hid_key_num_t key_num)
{
    ...
    switch (key_num)
    {
        ...
        break;
        case HID_KEY_NUMBLE_DISPLAY_Bri_IN:
        {
            hog_send_keyboard_display(ble_conn_handle, DISPLAY_BRI_IN);
            vTaskDelay(100/portTICK_RATE_MS);

            hog_send_keyboard_display(ble_conn_handle, 0X00);
            LOG_I("HID SEND:0X%02X", DISPLAY_BRI_IN);
        }
        break;
        case HID_KEY_NUMBLE_DISPLAY_Bri_DE:
        {
            hog_send_keyboard_display(ble_conn_handle, DISPLAY_BRI_DE);
            vTaskDelay(100/portTICK_RATE_MS);
            hog_send_keyboard_display(ble_conn_handle, 0X00);
            LOG_I("HID SEND:0X%02X", DISPLAY_BRI_DE);
        }
        break;
        default:
            break;
...
static void hid_send_task(void* arg)
{
    ...
    while (1) {
        xTaskNotifyWait(0xffffffff, 0, &kb_num, portMAX_DELAY);
...
void ble_hid_init()
{
    ...
    xTaskCreate(hid_send_task, "HID send", 1024, NULL, 7, &ble_send_task);
}

void ble_hid_notify_send(hid_key_num_t  kb_num)
{
    xTaskNotify(ble_send_task, kb_num, eSetValueWithOverwrite);
}

void ble_hid_enable(int bools)
{
    if (bools) {
        bt_enable(bt_enable_cb);
        ble_start_status = true;
        ...
        bt_disable();
        ble_kb_stop();
        LOG_I("BLE HID disanble");
        ble_start_status = false;
    }
}